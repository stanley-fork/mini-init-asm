...
set +e
timeout 10 env EP_RESTART_ENABLED=1 EP_MAX_RESTARTS=2 EP_RESTART_BACKOFF_SECONDS=0 $BIN -v -- /bin/bash -c '
...
EP_RESTART_ENABLED=1 EP_MAX_RESTARTS=1 EP_RESTART_BACKOFF_SECONDS=2 $BIN -v -- /bin/sh -c 'kill -SEGV $$' &
init_pid=$!
set +e
wait $init_pid
wait_rc=$?
...
EP_RESTART_ENABLED=1 EP_MAX_RESTARTS=2 EP_RESTART_BACKOFF_SECONDS=0 $BIN -v -- /bin/sh -c 'kill -SEGV $$' &
init_pid=$!
set +e
wait $init_pid
wait_rc=$?
...
EP_RESTART_ENABLED=1 EP_MAX_RESTARTS=10 EP_RESTART_BACKOFF_SECONDS=0 $BIN -v -- /bin/sh -c '
    trap "exit 0" TERM
    sleep 100 &
    sleep_pid=$!
...
# Run with timeout wrapper
set +e
timeout 10 env EP_RESTART_ENABLED=1 EP_MAX_RESTARTS=0 EP_RESTART_BACKOFF_SECONDS=0 $BIN -v -- /bin/sh -c '
